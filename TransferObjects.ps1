    ...
    if (-Not (Test-Path $SourceAccountLocatorOrFilepath )) {
        ...
        $SFObjectTypes -Split ',' | ForEach-Object {
            ...
            if ($obj -eq "all") {
                ...
                $SourceFilters = $AllFilters | Where-Object { 'datebucket','daterange','timezone' –notcontains $_.keyword }
...
Specifies the object type being updated. Filters, Dashboards, Worksheets, or or a combination of the three (do not use all with the others or duplicates will be created) can be entered.
...
A string with the account locator or file path of the source objects you want to transfer to target accounts. If supplied with a value that is not a file path, this will evaluate as an account locator and attempt to connect. See the -Connect-SFApp method for more details on how connections will be made.
...
A comma separated string with the account locators you wish to upload snowflake objects from SourceAccountLocatorOrFilepath to.
Ex: 'xy12345.us-east-1,MyGCPAcct.us-central1,xy12345.west-us-2.azure,MyPrivateLinkAcct.privatelink.snowflakecomputing.com'

.PARAMETER SFRole
Specifies the Snowflake Role required to run the specific object in new accounts. Default is ACCOUNTADMIN.

.PARAMETER SFWarehouse
Specifies the Snowflake Warehouse required to run the specific object in new accounts. An error will display if no warehouse is designated. Default is COMPUTE_WH

.PARAMETER SFDatabase
--SFDatabase [Optional, name of database to use in target accounts for context]

.PARAMETER SFSchema
--SFSchema [Optional name of schema to use in target accounts for context]

.PARAMETER OutputDirectory
--OutputDirectory [Optional string with a parent level dir determining where source and target accounts should create sub dirs]
...
function SSO-Prompt([string]$inputString){
    do {
        $UserInput = Read-Host -Prompt "`r`nDoes the account at $inputString use SSO? Use y/n, enter for default value of no."
    } while (
